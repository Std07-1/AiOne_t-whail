...
def _get_fetcher() -> OptimizedDataFetcher:
    ...
    if _FETCHER_SINGLETON is None:
        ...
        # OptimizedDataFetcher не приймає compress_cache у цій гілці репо
        _FETCHER_SINGLETON = OptimizedDataFetcher(session=session)
...
def fetch_bars(symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        if "timestamp" in df.columns:
            # Значення у мс → потрібен unit="ms"
            df.index = pd.to_datetime(df["timestamp"], unit="ms", utc=True)
        else:
            df.index = pd.to_datetime(df.index, unit="ms", utc=True, errors="coerce")
    df.index.name = "time"
