...
from config.flags import STAGE1_PREFILTER_REDIS_TRAP_ONLY, STAGE1_SKIP_FULL_FILTER
...
class BinanceFutureAssetFilter:
    ...
    # Зберігаємо останній список символів після базової фільтрації (prefilter)
    last_prefiltered: list[str] | None = None
    # Зберігаємо останній список символів після повної фільтрації (filtered)
    last_filtered_list: list[str] | None = None
    ...
    async def filter_assets(self, params: FilterParams) -> list[str]:
        ...
        # Radical variant: allow building prefilter purely from Redis-state + manual seed + whitelist
        if bool(STAGE1_PREFILTER_REDIS_TRAP_ONLY):
            logger.info(
                "STAGE1_PREFILTER_REDIS_TRAP_ONLY=True — будуємо prefilter з Redis-state/manual seed (пропускаємо 24h пороги)"
            )
            # Attempt to obtain candidate symbols from store (UnifiedDataStore.get_fast_symbols)
            candidates: list[str] = []
            try:
                if hasattr(self.store, "get_fast_symbols"):
                    fast = await self.store.get_fast_symbols()  # type: ignore[attr-defined]
                    if isinstance(fast, list) and fast:
                        candidates.extend(fast)
            except Exception:
                logger.debug(
                    "Не вдалося отримати fast_symbols з store (Redis) — фолбек"
                )

            # manual seed from config
            try:
                if (
                    isinstance(MANUAL_FAST_SYMBOLS_SEED, (list, tuple))
                    and MANUAL_FAST_SYMBOLS_SEED
                ):
                    candidates.extend(list(MANUAL_FAST_SYMBOLS_SEED))
            except Exception:
                pass

            # include explicit WHITELIST_SYMBOLS if present
            try:
                candidates.extend(list(WHITELIST_SYMBOLS))
            except Exception:
                pass
            ...
            if not cleaned:
                logger.warning(
                    "STAGE1_PREFILTER_REDIS_TRAP_ONLY увімкнений, але не знайдено candidate symbols у Redis/manual_seed/whitelist — продовжуємо стандартний шлях"
                )
                if params.dynamic:
                    ...
                    logger.debug(
                        "[EVENT] Динамічні пороги вимкнено, використовуємо статичні"
                    )
            else:
                # bypass 24h ticker-based prefilter: create minimal DataFrame with symbol column
                import pandas as _pd

                prefiltered_df = _pd.DataFrame({"symbol": cleaned})
                symbols = cleaned
                logger.info("Prefilter (redis/trap-only) candidates=%d", len(symbols))
                # advance progress to match skipped steps
                self.progress.advance(main_task)
                # jump to metric collection (step 4)
                # note: we don't change params here
                # proceed further with prefiltered_df and symbols
        else:
            if params.dynamic:
                params = await self.apply_dynamic_thresholds(ticker_df, params)
                logger.debug(f"[EVENT] Параметри після динаміки: {params.dict()}")
        ...
        # Зберігаємо список після базової фільтрації для зовнішнього доступу
        try:
            type(self).last_prefiltered = list(prefiltered_df["symbol"].tolist())
        except Exception:
            type(self).last_prefiltered = []
        ...
        # If configured to skip the full (heavy) filtering, return immediately
        # after the base prefilter so monitoring can run on the post-basic list.
        if bool(STAGE1_SKIP_FULL_FILTER):
            logger.info(
                "STAGE1_SKIP_FULL_FILTER=True — повертаємо список після базової фільтрації (без важких метрик)"
            )
            try:
                # save last_filtered_list as the same as prefiltered
                type(self).last_filtered_list = list(symbols)
            except Exception:
                type(self).last_filtered_list = []

            # Prepare a deterministic result ordering: prefer quoteVolume if present
            try:
                if "quoteVolume" in prefiltered_df.columns:
                    ordered = (
                        prefiltered_df.sort_values("quoteVolume", ascending=False)
                    ...
                    ordered = symbols[: params.max_symbols]
            except Exception:
                ordered = symbols[: params.max_symbols]

            # Create lightweight metrics snapshot
            elapsed = time.monotonic() - start_time
            metrics = MetricResults(
                initial_count=len(ticker_df),
                prefiltered_count=len(prefiltered_df),
                filtered_count=len(type(self).last_filtered_list or []),
            ...
            # print results via existing helper for consistency
            print_results(ordered, metrics)
            self.progress.stop()
            return ordered
        ...
        if filtered_df.empty:
            ...
            return []
        # Зберігаємо список після повної фільтрації
        try:
            type(self).last_filtered_list = list(filtered_df["symbol"].tolist())
        except Exception:
            type(self).last_filtered_list = []
        self.progress.advance(main_task)
