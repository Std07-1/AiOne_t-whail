...
class Thresholds:
    ...
    def __init__(
        ...
        """
        self.symbol = symbol
        self.low_gate = config.get("low_gate", 0.0035)
        self.high_gate = config.get("high_gate", 0.012)
        self.atr_target = config.get("atr_target", 0.35)
    ...
    def from_mapping(cls, data: dict) -> Thresholds:
        ...
        cfg: dict[str, Any] = {
            "low_gate": data.get("low_gate", 0.0035),  # Нижня межа ATR/price (0.35%)
            "high_gate": data.get("high_gate", 0.012),  # Верхня межа ATR/price (1.34%)
            "atr_target": float(
                data.get("atr_target", 0.35)
...
async def load_thresholds(
    ...
    return Thresholds.from_mapping(
        {
            "symbol": symbol,
            "low_gate": config.get("low_gate", 0.0035),
            "high_gate": config.get("high_gate", 0.012),
            "atr_target": config.get("atr_target", 0.35),
