...
# За свіченими прапорами метрики піднімаються на локальному HTTP‑ендпоінті.
# Логічний прапорець, який вмикає або вимикає експортування метрик у форматі Prometheus.
# Якщо True, застосунок піднімає HTTP‑ендпоінт для збору метрик.
# Якщо у вашому середовищі не використовується Prometheus або Pushgateway, цей прапорець можна вимкнути.
# Ці налаштування дозволяють легко інтегрувати систему з моніторингом Prometheus, не змінюючи основну логіку застосунку.
# Вимкніть, якщо не використовуєте Prometheus/Pushgateway у середовищі.
PROM_GAUGES_ENABLED: bool = True
# порт, на якому буде доступний HTTP‑ендпоінт для метрик Prometheus (за замовчуванням 9108).
PROM_HTTP_PORT: int = 9108
# Важливо: не залишайте увімкненим Prometheus у продакшн‑середовищі без потреби, щоб уникнути зайвого навантаження та відкритих портів.
