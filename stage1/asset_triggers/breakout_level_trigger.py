...
def breakout_level_trigger(
    ...
    *,
    ...
    # Виключаємо одночасну близькість до обох рівнів - нове, додано 2024-06-10
    if triggers["near_high"] and triggers["near_low"]:
        if abs(recent_high - current_close) <= abs(current_close - recent_low):
            triggers["near_low"] = False
            logger.debug(
                "[%s] Виключено near_low через ближчий recent_high (diff_high=%.4f, diff_low=%.4f)",
                symbol,
                abs(recent_high - current_close),
                abs(current_close - recent_low),
            )
        else:
            triggers["near_high"] = False
            logger.debug(
                "[%s] Виключено near_high через ближчий recent_low (diff_high=%.4f, diff_low=%.4f)",
                symbol,
                abs(recent_high - current_close),
                abs(current_close - recent_low),
            )
    if triggers["breakout_down"]:
        triggers["near_high"] = False
        logger.debug("[%s] Виключено near_high через активний breakout_down", symbol)
    if triggers["breakout_up"]:
        triggers["near_low"] = False
        logger.debug("[%s] Виключено near_low через активний breakout_up", symbol)
