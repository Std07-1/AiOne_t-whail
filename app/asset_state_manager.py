...
class AssetStateManager:
    ...
    def __init__(
        ...
        # Лічильник Stage2 таймаутів (task cancellations due to timeout)
        self.stage2_timeouts: int = 0
    ...
    def get_trade_ready_signals(self) -> list[dict[str, Any]]:
        """Повертає сигнали, що пройшли Stage2 та готові до Stage3 відкриття."""
        trade_ready: list[dict[str, Any]] = []
        for asset in self.state.values():
            signal = str(asset.get(K_SIGNAL, "")).upper()
            if signal not in {"ALERT_BUY", "ALERT_SELL"}:
                continue
            status = asset.get("stage2_status")
            if status != STAGE2_STATUS["COMPLETED"]:
                continue
    ...
    def finalize_alert_session(
        ...
        try:
            ...
            # Для Stage3: можна додати поле 'stage3_skip_reason' якщо потрібно
            # Централізоване зберігання у TELEMETRY_BASE_DIR
            from pathlib import Path

            try:
                from config.config import ALERTS_QUALITY_LOG, TELEMETRY_BASE_DIR
            except Exception:
                telemetry_base_dir, alerts_quality_log = (
                    "./telemetry",
                    "alerts_quality.jsonl",
                )
            else:
                telemetry_base_dir, alerts_quality_log = (
                    TELEMETRY_BASE_DIR,
            ...
            out_path = Path(telemetry_base_dir) / alerts_quality_log
