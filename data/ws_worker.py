...
# ────────────────────── WS Worker ──────────────────────
class WSWorker:
    """WebSocket-воркер для 1m Binance klines у UnifiedDataStore.

    Legacy RAM‑буфери видалені: хвилинні бари зберігаються через
    ``store.put_bars``; актуальні значення доступні через Redis усередині
    ``UnifiedDataStore``.

    Args:
        symbols: Початковий вайтліст символів у нижньому регістрі. Якщо не
            задано — буде використано `DEFAULT_SYMBOLS`.
        store: Інстанс сховища даних (єдиний джерело правди).
        selectors_key: Ключ селектора у просторі імен сховища (через двокрапку),
            напр. "selectors:fast_symbols". Якщо не задано — використовується
            ``store.get_fast_symbols``.
        intervals_ttl: Користувацьке TTL для легасі-снапшотів 1m/1h у Redis.

    Raises:
        ValueError: Якщо ``store`` не передано.
    """
    ...
    async def _get_live_symbols(self) -> list[str]:
        ...
        if not syms:
            if self._symbols:
                ...
                logger.warning(
                    "[WSWorker] selector:active:stream порожній або некоректний, fallback btcusdt"
                )
    ...
    async def _handle_kline(self, k: dict[str, Any]) -> None:
        ...
        if is_closed:
            ...
            # Reactive Stage1: виклик монітора на закритті кожного 1m бару + публікація снапшоту UI
            try:
                # Читаємо фіче-флаг із конфігурації (з можливістю оверрайду через env)
                try:
                    from config.config import REACTIVE_STAGE1  # type: ignore
                ...
                if reactive and monitor is not None:
                    # Не блокуємо WS-цикл: викликаємо монітор у фоні
                    asyncio.create_task(self._reactive_stage1_call(monitor, sym, None))

                    # Best‑effort публікація агрегованого стану для UI
                    try:
                        state_manager = getattr(monitor, "state_manager", None)
                        redis_pub = getattr(
                            getattr(self.store, "redis", None), "r", None
                        )
                        if state_manager is not None and redis_pub is not None:
                            from UI.publish_full_state import (
                                publish_full_state,  # локальний імпорт для уникнення циклів
                            )

                            asyncio.create_task(
                                publish_full_state(state_manager, self.store, redis_pub)
                            )
                    except Exception:
                        # не провалюємо основний цикл через публікацію
                        pass
            except Exception:
                # захисний бар'єр: проблеми в реактивній гілці не мають впливати на WS
                pass
    ...
    async def consume(self) -> None:
        ...
        Side effects:
            - Мережеві з'єднання до Binance WS.
            - Читання/запис у Redis через ``UnifiedDataStore``.
            - Фонові задачі для оновлення вайтліста символів.
        """
        while not self._stop_event.is_set():
            try:
