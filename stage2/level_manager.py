...
Призначення: централізоване управління та збагачення рівнів для історичного QDE та Stage2.
"""
    ...
    def get_corridor(
        ...
        if support is not None and resistance is not None and resistance > support:
            ...
            if s_obj and r_obj:
                ...
                # Обмежуємо надмірно широкі коридори до 0.3% для стабільніших гейтів
                try:
                    band_pct = float(min(band_pct, 0.3))
                except Exception:
                    pass
                confidence = float(
        ...
        if support is not None and resistance is not None and resistance > support:
            ...
            # Додаткова нормалізація band_pct зверху (0.3%)
            try:
                band_pct = float(min(band_pct, 0.3))
            except Exception:
                pass
