...
def ensure_active_symbols_preserved(
    current_symbols: set[str],
    active_symbols: set[str],
) -> tuple[set[str], set[str]]:
    """Гарантує, що активні угоди залишаються у whitelist fast_symbols.

    Args:
        current_symbols: Поточний набір символів (очікується нижній регістр).
        active_symbols: Символи з відкритими угодами (формат довільний).

    Returns:
        Кортеж (оновлений набір символів, множина символів, яких бракувало до злиття).
    """

    normalized_current = {str(sym).lower() for sym in current_symbols if sym}
    normalized_active = {str(sym).lower() for sym in active_symbols if sym}
    missing = normalized_active - normalized_current
    if not missing:
        return normalized_current, set()
    updated = normalized_current | normalized_active
            ...
            if combined_list:
                ...
                if trade_manager is not None:
                    try:
                        ...
                        logger.warning(
                            "Не вдалося отримати активні угоди для захисту whitelist",
                            extra={"batch_id": batch_id, "error": str(exc)},
                        )
                    else:
                        active_symbols = {
                            str(trade.get("symbol", "")).lower()
                            for trade in active_trades
                            if trade.get("symbol")
                        }
                        if active_symbols:
                            current_symbols, missing_before = (
                                ensure_active_symbols_preserved(
                                    current_symbols, active_symbols
                                )
                            )
                            if missing_before:
                                logger.info(
                                    "Prefilter: додано активні символи, щоб захистити відкриті угоди",
                                    extra={
                                        "batch_id": batch_id,
                                        "protected_count": len(active_symbols),
                                        "protected_added": sorted(list(missing_before))[
                                            :6
                                        ],
                                    },
                                )
