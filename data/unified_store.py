...
class RedisAdapter:
    ...
    def _build_key(self, *parts: str) -> str:
        """Будує Redis-ключ, коректно обробляючи як відносні, так і абсолютні частини.

        Логіка:
        - Якщо перша частина вже містить namespace у форматі "{namespace}:...", вважаємо це
          абсолютним ключем і просто додаємо інші частини через ':' без дублювання namespace.
        - Інакше — будуємо ключ через стандартний k(namespace, *parts).

        Args:
            *parts: Частини ключа (без namespace або вже повний ключ з namespace).

        Returns:
            Повний Redis-ключ.
        """
        if not parts:
            return self.cfg.namespace
        first = parts[0] or ""
        ns_prefix = f"{self.cfg.namespace}:"
        if isinstance(first, str) and first.startswith(ns_prefix):
            # Вже абсолютний ключ із namespace — не дублюємо префікс
            return ":".join([p for p in parts if p])
        # Відносний шлях — додаємо namespace за допомогою k()
        return k(self.cfg.namespace, *parts)

    async def jget(self, *parts: str, default: object | None = None) -> object | None:
        key = self._build_key(*parts)
        # RAM-only / Redis-optional режим: якщо ретраї вимкнені — миттєво повертаємо default
        if self.cfg.io_retry_attempts <= 0:
            return default
    ...
    async def jset(self, *parts: str, value: object, ttl: int | None = None) -> None:
        ...
        # RAM-only / Redis-optional режим: якщо ретраї вимкнені — no-op
        if self.cfg.io_retry_attempts <= 0:
            return
        for attempt in range(self.cfg.io_retry_attempts):
            try:
...
class StorageAdapter:
    ...
    async def save_bars(self, symbol: str, interval: str, df: pd.DataFrame) -> str:
        ...
        # Захист від pandas RuntimeWarning: чистимо числові OHLCV перед серіалізацією
        try:
            df = sanitize_ohlcv_numeric(
                df, logger_obj=logger, log_prefix=f"[save_bars:{symbol}:{interval}] "
            )
        except Exception:
            # best-effort: якщо санітизація впала, пишемо як є (не ламаємо пайплайн)
            pass
...
class UnifiedDataStore:
    ...
    def __init__(self, *, redis: Redis[Any], cfg: StoreConfig | None = None) -> None:
        ...
        # RAM-only fallback для списку fast symbols (коли Redis недоступний)
        self._fast_symbols_ram: list[str] = []
    ...
    async def set_fast_symbols(self, symbols: list[str], ttl: int = 600) -> None:
        ...
        """
        # Оновлюємо RAM-кеш незалежно від стану Redis
        self._fast_symbols_ram = [str(sym).lower() for sym in symbols if sym]
        # Пишемо у Redis (no-op у RAM-only режимі)
        await self.redis.jset(
            "selectors",
            "fast_symbols",
            value=self._fast_symbols_ram,
            ttl=ttl,
        )

    async def get_fast_symbols(self) -> list[str]:
        """Повертає перелік символів із префільтра, або порожній список."""
        # Спершу RAM-кеш (актуальний для поточного процесу)
        if self._fast_symbols_ram:
            return list(self._fast_symbols_ram)
        # Потім Redis (може бути недоступним → default)
    ...
    async def fetch_from_cache(
        ...
        *,
        ...
        """
        # RAM-only режим: миттєво повертаємо None без звернень до Redis
        if self.cfg.io_retry_attempts <= 0:
            return None
